...
const ListShipmentContainer = (props) => {
    const items = props.data.map((shipment, i) => {
        return (
            <ListShipmentCard destino={shipment.Destination} progress={shipment.ShippingStatusHistories}
                              tipo={shipment.Connection.Modality.modalityName} fecha={new Date(shipment.date)} key={i}
                              id={shipment.shippingId} isGrey={i % 2 === 1}/>
        );
    });
    return (
